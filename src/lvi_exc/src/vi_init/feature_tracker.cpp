...
bool inBorder(const cv::Point2f &pt){
  ...
  return BORDER_SIZE <= img_x && img_x 
    < COL - BORDER_SIZE && BORDER_SIZE <= img_y && img_y < ROW - BORDER_SIZE;
}
...
void FeatureTracker::rejectWithF(){
  if (forw_pts.size() >= 8){
    ...
    for (unsigned int i = 0; i < cur_pts.size(); i++){
      ...
      //这里的计算有错误吧？感觉上应该是【tmp_p.x() / tmp_p.z()，tmp_p.y() / tmp_p.z()】
  ...
  for (int i = 0; i < int(undistortedp.size()); i++){
    ...
    if (pp.at<float>(1, 0) + 300 >= 0 
      ...
      && pp.at<float>(0, 0) + 300 < COL + 600){
        undistortedImg.at<uchar>(pp.at<float>(
          1, 0) + 300, pp.at<float>(0, 0) + 300) 
