...
bool InitialEXRotation::CalibrationExRotationLiDAR(
      Matrix3d delta_R_lidar, 
      Quaterniond delta_q_imu, Matrix3d &calib_ric_result){
  frame_count++;
  // frame_count=1;
  Rc.push_back(delta_R_lidar);
...
//用四元数表示手眼标定的旋转方程，然后根据左右乘可以把方程化为与外参四元数相关的线性方程
...
double InitialEXRotation::testTriangulation(
    const vector<cv::Point2f> &l, const vector<cv::Point2f> &r,
