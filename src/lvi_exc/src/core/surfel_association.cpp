...
void SurfelAssociation::associateVisualPointsWithPlanes(
    std::shared_ptr<TrajectoryManagerLVI> traj_manager,
    const Eigen::Quaterniond& q_LtoC,
    const Eigen::Vector3d& t_LinC,
    const std::map<int64_t, std::shared_ptr<kontiki::sfm::Landmark>>& landmarks,
    std::map<kontiki::sfm::Landmark*, size_t>& lm_splane){
  //tranverse all landmarks, recover their 3d locations 
  //and assign associated surfel (if have).

  //Approximate LiDAR pose at lidar mapping stamp.
  Eigen::Quaterniond q_CtoG, q_L0_G;
  Eigen::Vector3d p_CinG, t_L0_G;
  if(!traj_manager->evaluateCameraPose(map_timestamp_, q_CtoG, p_CinG)){
    LOG(INFO)<<map_timestamp_;
    return;
  }
  q_L0_G = q_CtoG * q_LtoC;
  t_L0_G = q_CtoG * t_LinC + p_CinG;

  std::shared_ptr<kontiki::sensors::PinholeCamera> camera = 
    traj_manager->getCameraModel();
  for(auto it = landmarks.begin(); it != landmarks.end(); it++){
    for (int surfel_idx = 0; surfel_idx < surfel_planes_.size(); surfel_idx++) {
      ...
      Eigen::Vector3d p3d_C, p3d_G, p3d_L0;
      p3d_C = camera->Unproject(it->second->reference()->uv());
      if(it->second->inverse_depth() < 0.05) continue;//beyond 20 meters
      p3d_C = p3d_C / it->second->inverse_depth();
      
      double timestamp = it->second->reference()->view()->t0();
      if(!traj_manager->evaluateCameraPose(timestamp, q_CtoG, p_CinG)){
        continue;
      }
      p3d_G = q_CtoG * p3d_C + p_CinG;
      p3d_L0 = q_L0_G.inverse() * (p3d_G - t_L0_G);

      if(p3d_L0[0] > box_min[0] && p3d_L0[0] < box_max[0] &&
        p3d_L0[1] > box_min[1] && p3d_L0[1] < box_max[1] &&
        p3d_L0[2] > box_min[2] && p3d_L0[2] < box_max[2]) {
        
        double dst = point2PlaneDistance(p3d_L0, plane_coeffs);
        LOG(INFO)<<p3d_L0.transpose()<<", "<<dst;
        if (dst <= associated_radius_*2) {
          // mask_per_ring.push_back(i);
          // lm_splane[it->second.get()] = surfel_planes_.at(surfel_idx);
          lm_splane[it->second.get()] = surfel_idx;
        }
      }
    }
  }
