...
CalibrHelper::CalibrHelper(ros::NodeHandle& nh)
...

  {
    ...
    dataset_reader_->read(bag_path_, topic_imu_, topic_lidar, topic_image, bag_start, bag_durr);
  ...
  licalib::CameraIntrinsic camera_params;
  traj_manager_ = std::make_shared<TrajectoryManagerLVI>(
    camera_params, map_time_, end_time, knot_distance, time_offset_padding);
...
//作帧间NDT matching，初始化雷达和IMU外参
bool CalibrHelper::Initialization() {
...
void CalibrHelper::Mapping(bool relocalization) {
  ...
  if (relocalization) {
    ...
    scan_undistortion_->undistortScan(false);//初始时未作矫正
    lidar_odom_ = std::make_shared<LiDAROdometry>(ndt_resolution_);
  }
