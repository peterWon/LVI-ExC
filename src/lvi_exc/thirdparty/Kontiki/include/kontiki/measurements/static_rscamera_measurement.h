...
Eigen::Matrix<T, 2, 1> reproject_static(const sfm::Observation& ref,
  ...
  // T t_ref = ref.view()->t0() + time_offset;
  // T t_obs = obs.view()->t0() + time_offset;
  ...
  const Vector3 p_CinI = camera.relative_position();
  const Eigen::Quaternion<T> q_CinI = camera.relative_orientation();
  
  //注意：这里的相对位姿意义跟LiDAR Surfel里面是反过来的，这里表示的是IMU在Camera下的姿态
  const Vector3 p_ct = q_CinI.conjugate()*(-p_CinI);
  const Eigen::Quaternion<T> q_ct = q_CinI.conjugate();
   ...
   protected:
    ...
    void AddToEstimator(kontiki::TrajectoryEstimator<TrajectoryModel>& estimator) {
      ...
      if (!camera->TimeOffsetIsLocked()) {
        ...
        // std::cout<<camera->max_time_offset()<<std::endl;
      }

      const double margin = 1e-3;
      
      //B-Spline可以做到只更新有关片段，而保持其余不变（局部性）
      ...
      // Add camera to probleｍ, 每一个位置（关键帧时刻）都会对相机外参优化吗？不会，因为所添加的参数块其实指向的是同一个内存地址
