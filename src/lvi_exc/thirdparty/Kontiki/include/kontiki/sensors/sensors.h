...
struct SensorMeta : public entity::MetaData {
  size_t NumParameters() const override {
    return 3; // Relative pose (2) + time offset (1),　指的是数据块的个数
  }
...
class SensorEntity : public type::Entity<ViewTemplate, MetaType, StoreType> {
 public:
  //初始化的时候开辟待优化的参数内存块，每一个传感器默认有一个相对于参考系(应该是IMU)的旋转、位移及时间偏移量(参考点存疑，应该也是IMU的时间戳)
  //参数内存块同时也是待优化的，它与Ceres优化的对象共享内存
  SensorEntity() :
