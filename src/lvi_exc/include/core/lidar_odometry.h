...
public:
  ...
  const std::map<int64_t, OdomData> &get_odom_data_map() const {
    return odom_data_map_;
  }

  const std::vector<size_t>& getKeyFrameIndex() const{
    return key_frame_index_;
  }
...
private:
  ...
  std::map<int64_t, OdomData> odom_data_map_;
};
