...
public:
  ...
  CalibParamManager() :
    ...
    global_opt_gyro_weight = 28.5;
    global_opt_acce_weight = 18.5;
    global_opt_lidar_weight = 10.0;
    global_opt_visual_surfel_weight = 200.;

    global_opt_pos_weight = 1;
    global_opt_rot_weight = 0.5;
    global_opt_cam_weight = 1.0;//TODO(wz)
  }
  ...
  void set_q_CtoI(Eigen::Quaterniond q) {
    q_CtoI = q;
  }

  void set_p_CinI(Eigen::Vector3d p) {
    p_CinI = p;
  }
  ...
  void showStates(bool show_camera) const {
    ...
    if(show_camera){
      Eigen::Vector3d euler_CtoI = q_CtoI.toRotationMatrix().eulerAngles(0,1,2);
      euler_CtoI = euler_CtoI * 180 / M_PI;

      Eigen::Quaterniond q_ItoC = q_CtoI.inverse();
      Eigen::Vector3d p_IinC = q_ItoC * (-p_CinI);
      Eigen::Vector3d euler_ItoC = q_ItoC.toRotationMatrix().eulerAngles(0,1,2);
      euler_ItoC = euler_ItoC * 180 / M_PI;

      std::cout << "P_CinI      : " << p_CinI.transpose() << std::endl;
      std::cout << "euler_CtoI  : " << euler_CtoI.transpose() << std::endl;
      std::cout << "P_IinC      : " << p_IinC.transpose() << std::endl;
      std::cout << "euler_ItoC  : " << euler_ItoC.transpose() << std::endl;
    }
...
public:
  ...
  Eigen::Vector3d p_CinI; //wz add

  Eigen::Quaterniond q_LtoI;

  Eigen::Quaterniond q_CtoI;//wz add
  ...
  double global_opt_visual_surfel_weight;

  double global_opt_cam_weight;

  double global_opt_pos_weight;//lidar pose solved from LOAM
  double global_opt_rot_weight;//lidar pose solved from LOAM
};
