...
public:
  ...
  explicit ScanUndistortion(TrajectoryManagerLVI::Ptr traj_manager,
  ...
  void undistortScan(bool correct_position = false) {
    ...
    for (const TPointCloud& scan_raw: dataset_reader_->get_scan_data()) {
      ...
      double scan_timestamp = 
        pcl_conversions::fromPCL(scan_raw.header.stamp).toSec();
      if (!traj_manager_->evaluateLidarPose(
          scan_timestamp, q_L0_to_G, p_L0_in_G)) {
  ...
  void undistortScanInMap(
    ...
    for (size_t idx = 0; idx < dataset_reader_->get_scan_data().size(); idx++){
      ...
      pcl::transformPointCloud(
        *(iter->second), *scan_inMap, odom_data.at(idx).pose);
  ...
  void undistortScanInMap(
      const std::map<int64_t, LiDAROdometry::OdomData>& odom_data_map) {
    scan_data_in_map_.clear();
    map_cloud_ = VPointCloud::Ptr(new VPointCloud);
    ...
    for (size_t idx = 0; idx < dataset_reader_->get_scan_data().size(); idx++){
      ...
      double t_pcl = pcl_conversions::fromPCL(scan_raw.header.stamp).toSec();
      int64_t t_64 = t_pcl*1e9;
      auto iter_pose = odom_data_map.find(t_64);
      if(iter_pose == odom_data_map.end()) continue;
      VPointCloud::Ptr scan_inMap = VPointCloud::Ptr(new VPointCloud);
      pcl::transformPointCloud(
        *(iter->second), *scan_inMap, (iter_pose->second).pose);
      scan_data_in_map_.insert({scan_raw.header.stamp, scan_inMap});
      *map_cloud_ += *scan_inMap;
    }
  }
...
private:
  //根据当前所估计的轨迹，把在scan_raw下的点转换到目标时刻下的点云（scan_in_target）
  void undistort(const Eigen::Quaterniond& q_G_to_target,
    ...
    for (int h = 0; h < scan_raw.height; h++) {
      for (int w = 0; w < scan_raw.width; w++) {
        VPoint vpoint;
        int idx = h * scan_raw.width + w;
        if (pcl_isnan(scan_raw.points.at(idx).x)) {
          vpoint = NanPoint;
          scan_in_target->at(w,h) = vpoint;
          continue;
        }
        double point_timestamp = scan_raw.points.at(idx).timestamp;
        ...
        Eigen::Vector3d p_Lk(scan_raw.points.at(idx).x, scan_raw.points.at(idx).y, scan_raw.points.at(idx).z);
        ...
        vpoint.intensity = scan_raw.points.at(idx).intensity;
        scan_in_target->at(w,h) = vpoint;
      }
    }
  }
